...
pnpm run test
pnpm run test:ext
PUBLIC_RELEASE=true EXPECTED_PUBLISHER=Tamzeeho EXPECTED_VERSION=0.6.6 pnpm run release:check
pnpm run package
pnpm dlx @vscode/vsce package --no-dependencies --out zeeho-git-graph-0.6.6.vsix
unzip -t zeeho-git-graph-0.6.6.vsix
shasum -a 256 zeeho-git-graph-0.6.6.vsix
```
...
4. Create the tag only after CI is green:
   ...
   ```sh
   git tag -s v0.6.6 -m "Zeeho Git Graph v0.6.6"
